...
class LisTLLProtocol:
    ...
    "EiAr_100": {"level": 1, "cyber_pos": 'zebbb810.ala.dedicated.aliyun.emqxcloud.cn', "real_pos": "None", "tll_port": 8080,
    ...
    def request(self, command, name, function):
        ...
        if self.access_list[name]["cyber_pos"] == self.cyber_pos:
            self.socket.publish(pos+port, text.encode("utf-8"), qos=1)
        else:
            k = self.access_list[name]["cyber_pos"]
            self.logger(f"{k} is not at the default server, creating new socket...", "WARNING")
            _socket = mqtt_client.Client(mqtt_client.CallbackAPIVersion.VERSION2, self.name + str(random.randint(1, 10000)))
    ...
    def tell_to_NLP(self, msg):
        self.socket.publish(self.name+str(self.nlp_port), msg.encode("utf-8"), qos=1)
        time.sleep(0.5)

    def tell_to_TSK(self, msg):
        self.socket.publish(self.name+str(self.tsk_port), msg.encode("utf-8"), qos=1)
        time.sleep(0.5)
        ...
        if language == "TLL":
            ...
            if receiver == "Natual":
                ...
                if "【发送通知】" in msg and self.llm_linguistic_mode:  # 模型调用开关
                    ...
                    self.logger(f"机器人触发TLL标签：{ans}")
                    try:
                        self.tell_to_TLL(self.Natual_to_TLL(ans))
                    except:
                        self.logger(f"TLL转写失败。转为Natual", "WARNING")
    ...
    def TSK_receive(self):
        ...
        self.socket.subscribe(self.name+str(self.tsk_port), qos=1)
    ...
    def NLP_receive(self):
        self.socket.subscribe(self.name+str(self.nlp_port), qos=1)
        self.logger(self.name + f": NLP open in topic[{self.name + str(self.nlp_port)}]", "SUCCESS")

    def user_input(self):
        self.logger("\033[0;32m[->] TSK启用录音功能\033[0m" if self.listen_access else "\033[0;31m[->] TSK禁用录音功能\033[0m")
        self.logger("\033[0;32m[->] TSK启用语音功能\033[0m" if self.speak_access else "\033[0;31m[->] TSK禁用语音功能\033[0m")
    ...
    def TLL_boot(self):
        self.socket.subscribe(self.name + str(self.tll_port), qos=1)
