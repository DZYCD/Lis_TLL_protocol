...
def get(arg):
    if arg[0] == 'access_list':
        time.sleep(3)
        return "202+" + f"dos+access_update({str(access_list)})"
    else:
        return "503"
```
...
# 示例
## SaYi_SV
SaYi_SV 包含了语音输入、语音输出、关键词控制和模型控制功能，并且负责发放权限表。
...
```
## SaYi_900
SaYi_900 是控制电脑软件的脚本，接入了协议后可以允许其他机器人打开电脑软件。
...
```
## SaYi_991
SaYi_991 是允许进行数据处理的机器人，她可以接收摄像头端的数据进来，进行过滤后输出
...
```
## Skaye_800
Skaye_800 是允许监控机器人。他可以监控并识别人体摔倒或站立。
...
## SaYi_992
SaYi_992是部署在单片机上的机器人。他可以进行灯光的控制
